...
export const signin = async (req, res,next) => {
    ...
    //cookie
    res.cookie("access_token", token, { httpOnly: true ,expires: new Date(Date.now() + 8 * 3600000)});
...
export const google = async (req, res,next) => {
    try{
        ...
        if(user){
            ...
            const newuser = new User({ username:req.body.name.split(" ").join("").toLowerCase(), email:req.body.email, password:hashpassword,avatar:req.body.photo });
            await   newuser.save();
            console.log("User created successfully");
